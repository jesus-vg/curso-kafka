package com.desv4j.kafka.callbacks;
...
public class Devs4jCallbackProducer {
	...
	private static Logger log = LoggerFactory.getLogger( Devs4jCallbackProducer.class );
	...
	public static void main( String[] args ) {
		...
		try ( Producer<String, String> producer = new KafkaProducer<String, String>( props ) ) {
			for ( int i = 0; i < 1_000; i++ ) {
				...
				producer.send(
					new ProducerRecord<>( "devs4j-topic", i + "", "message " + ( i + 1 ) ), ( recordMetadata, e ) -> {
						if ( e != null ) {
							log.info( "There was an error {}", e.getMessage() );
						}
						
						log.info(
							"Ofset = {}, Partition = {}, Topic = {}",
